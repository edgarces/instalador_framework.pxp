...
# cambio de los archivos pg_hba y postgres.config#
	...
	run('psql -U postgres -c "CREATE DATABASE dbkerp WITH ENCODING=\'UTF-8\';"')
	run('psql -U postgres -c "CREATE USER dbkerp_conexion WITH PASSWORD \'dbkerp_conexion\';"')
	run('psql -U postgres -c "CREATE USER dbkerp_admin WITH PASSWORD \'a1a69c4e834c5aa6cce8c6eceee84295\';"')
	run('psql -U postgres -c "CREATE ROLE rol_usuario_dbkerp SUPERUSER NOINHERIT ROLE dbkerp_admin;"')
